(ns cegid_retail.main
  "Kotodama WASM entrypoint for the cegid_retail clean-room actor (L4) — Clojure port.
  ...
  facts (`cegid_retail.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cegid_retail")
(def tier "L4")
...
(def entity-specs
  [{:entity "Product"    :plural "products"    :id-prefix "cegidret_pro"
   ...
   {:entity "Customer"   :plural "customers"   :id-prefix "cegidret_cus"
    :fields [:email :firstName :lastName :phone] :required [:email :firstName]
    :coerce {}                       :refs {}}
   {:entity "Collection" :plural "collections" :id-prefix "cegidret_col"
...
(defn emit-facts
  "EAVT facts for one record: {\"cegid_retail.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cegid_retail-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cegid_retail-compat/ready)
