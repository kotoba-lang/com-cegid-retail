(ns cegid_retail.main-test
  (:require [clojure.test :refer [deftest is testing run-tests]]
            [clojure.string :as str]
            [cegid_retail.main :as m]))
...
(deftest healthz
  (let [[body status] (m/healthz)]
    (is (= 200 status)) (is (= "cegid_retail-compat" (:actor body))) (is (= (set m/entities) (set (:entities body))))))

#?(:clj (defn -main [& _]
          (let [{:keys [fail error]} (run-tests 'cegid_retail.main-test)]
